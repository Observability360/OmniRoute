...
async function handleComboChatInner({
  ...
  // Explicit targets take precedence over pins and every normal combo strategy.
        ...
        const executeTarget = async (
          ...
          if (cbStatus.state === "OPEN") {
            ...
            if (
              ...
              (earliestCircuitOpenRetryMs === 0 ||
                cbStatus.retryAfterMs < earliestCircuitOpenRetryMs)
            ) {
          ...
          if (
            ...
            log.info(
              "COMBO",
              `Skipping ${modelStr} — model locked by resilience (cooldown active)`
            );
            recordComboDecision(traceInvocationId, {
              step: target.executionKey,
              target: modelStr,
          ...
          for (let retry = 0; retry <= maxRetries; retry++) {
            ...
            if (signal?.aborted) {
              log.info(
                "COMBO",
                `Client disconnected — aborting combo loop before model ${modelStr}`
              );
            ...
            if (
              ...
              log.warn(
                "COMBO",
                `Model ${modelStr} failed with body-specific error, stopping combo`
              );
              clearStaleLKGP(combo.name, target.executionKey, combo.id, log, "COMBO");
